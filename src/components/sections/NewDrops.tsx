    ...
    return (
        <section className="mt-16">
            ...
            <div className="flex items-end justify-between mb-8">
                <h2 className="text-5xl md:text-6xl lg:text-[80px] font-semibold uppercase leading-[0.9]  text-primary">
                    DON&rsquo;T MISS OUT
                    <br />
                    NEW DROPS
                </h2>
                <button className="bg-secondary text-white text-xs md:text-sm font-medium uppercase tracking-wider py-3 px-6 rounded-lg hover:bg-[#3a3a38] transition-colors shrink-0">
                    Shop New Drops
                </button>
            </div>
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5 justify-items-center">
                    {products?.map((product) => (
                        <div
                            key={product.id}
                            className="group cursor-pointer transition-transform duration-300 hover:scale-[1.02] flex flex-col p-2 w-full"
                        >
                            {/* Image Container */}
